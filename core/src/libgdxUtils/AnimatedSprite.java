 ...
 * to the information in the {@link Animation}.<br>
 * Usage:
 * <p>
 * <code>Animation animation = new Animation(1 / 3f, frame1, frame2, frame3);<br>
...
public class AnimatedSprite extends Sprite {
    ...
    /**
     * if the size should be set to the each frame's
     * {@link TextureRegion#getRegionWidth() region size}
     */
    private boolean useFrameRegionSize;

    /**
     * if the position should be set to let the each frame's center coincide
     * with the center of the previous frame
     */
    ...
    /**
     * {@link Sprite#draw(Batch) Draws} this {@code AnimatedSprite}. If
     * {@link #autoUpdate} is true, {@link #update()} will be called before
     * drawing.
     */
    @Override
    ...
    /**
     * Flips all frames from {@code startTime} to {@code endTime}. Note the
     * actual TextureRegions are
     * {@link TextureRegion#flip(boolean, boolean) flipped}, so if the
     * {@link #animation} contains a region more than once, those frames cannot
     * be flipped differently at the same time. Also they will be flipped as
     * often as they occur in the given time range.
     *
     ...
     * @param set if the frames should be set to {@code flipX} and {@code flipY}
